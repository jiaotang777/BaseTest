...
function ansiReportToHtml(value) {
  return String(value || "")
    .split("\n")
    .map((rawLine) => {
      ...
      // NodeQuality 各质量报告标题真正居中。
      // white-space:normal 会消除上游为了终端排版加的首尾空格。
      if (
        /^(?:硬件质量体检报告|IP质量体检报告(?:\(Lite\))?|网络质量体检报告|HARDWARE\s+QUALITY.*REPORT|IP\s+QUALITY.*REPORT|NET(?:WORK)?\s+QUALITY.*REPORT)\s*[:：]/i
          .test(plain)
      ) {
        return `<span class="report-title-line">${ansiToHtml(rawLine)}</span>`;
      }

      return ansiToHtml(rawLine);
    })
    .filter((line) => line !== "")
    .join("\n");
}
...
function sectionMarkup(
  ...
  return `
    ...
    >
      ...
      <pre class="report-output${content ? "" : " empty-output"}">${content ? (source === "NodeQuality" ? ansiReportToHtml(content) : ansiToHtml(content)) : ""}</pre>
  ...
  if (v4) {
    cards.push(`
      <div class="ip-subcard">
        ...
        <pre class="report-output ip-suboutput">${ansiReportToHtml(v4)}</pre>
      </div>
    `);
  }
  ...
  if (v6) {
    cards.push(`
      <div class="ip-subcard">
        ...
        <pre class="report-output ip-suboutput">${ansiReportToHtml(v6)}</pre>
      </div>
    `);
  }
...
function reportPage(report) {
  ...
  return pageShell(`
    ...
    <script src="/assets/report.js?v=0418" defer></script>
  `, "BaseTest Report");
}
